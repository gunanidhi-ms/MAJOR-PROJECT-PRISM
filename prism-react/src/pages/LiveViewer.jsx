...
export default function LiveViewer() {
  ...
  const [datasetDir, setDatasetDir] = useState('E:\\MAJOR-PROJECT-PRISM\\phase1_ingestion\\sample_dicoms\\test_series')
  ...
  const [signature, setSignature] = useState('')  // Image display state
    ...
    try {
      ...
      ws.onmessage = (event) => {
        try {
          ...
          if (alert.is_pipeline_event) {
            setPipelineEvent(alert)
            return
          }
  ...
  const fetchDraftReport = async (studyId) => {
    try {
      setReportLoading(true)
      const res = await fetch(`http://localhost:8000/api/v1/report/${studyId}`)
      if (res.ok) {
        const data = await res.json()
        setDraftReport(data)
        setShowReviewModal(true)
      } else {
        alert('Failed to fetch draft report.')
      }
    } catch (e) {
      alert('Network error fetching draft report.')
    } finally {
  ...
  const handleSignReport = async () => {
    if (!signature.trim()) {
      alert('Please enter a digital signature.')
      return
    }
    try {
      setReportLoading(true)
      const res = await fetch(`http://localhost:8000/api/v1/sign-report`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          study_id: draftReport.study_id,
          radiologist_id: signature
        })
      })
      if (res.ok) {
        alert('Report successfully signed and finalized.')
      ...
      alert('Network error signing report.')
    } finally {
      setReportLoading(false)
    }
  }
  ...
  return (
    <div className="app-shell">
        ...
        <aside className="slice-panel">
          ...
          <div className="scanner-controls">
            ...
            <div className="scanner-controls__body">
              ...
              <div>
                <label style={{ display: 'block', fontSize: '11px', color: 'var(--text-muted)', marginBottom: '4px' }}>Patient Name (Optional)</label>
                <input 
                  type="text" 
                  value={patientName}
              ...
              <div style={{ display: 'flex', gap: '8px' }}>
                <div style={{ flex: 1 }}>
                  <label style={{ display: 'block', fontSize: '11px', color: 'var(--text-muted)', marginBottom: '4px' }}>Age (Opt)</label>
                  <input 
                    type="text" 
                    value={patientAge}
                ...
                <div style={{ flex: 1 }}>
                  <label style={{ display: 'block', fontSize: '11px', color: 'var(--text-muted)', marginBottom: '4px' }}>Sex (Opt)</label>
                  <select 
                    value={patientSex}
      ...
      {/* ── Pipeline Progress Floating Panel ── */}
      {pipelineEvent && pipelineEvent.status !== 'failed' && (
        <div className="pipeline-panel">
          <div className="pipeline-panel__header">
            Backend Progress
          </div>
          <div className="pipeline-panel__body">
            {pipelineEvent.status === 'completed' && pipelineEvent.phase === 'reporting' ? (
              <div className="pipeline-panel__success">
                <span className="pipeline-panel__icon">✅</span>
                <span>Draft Report Ready</span>
                <button 
                  className="pipeline-panel__btn"
                  onClick={() => fetchDraftReport(pipelineEvent.study_id)}
                >
                  Review & Sign
                </button>
              </div>
            ) : (
              <div className="pipeline-panel__loading">
                <div className="spinner"></div>
                <div className="pipeline-panel__text">
                  {pipelineEvent.phase === 'segmentation' ? 'Phase 2: Segmenting & Analyzing...' : 'Phase 3: AI Generating Draft Report...'}
                </div>
                {pipelineEvent.estimated_time_sec > 0 && (
                  <div className="pipeline-panel__eta">
                    Est: {pipelineEvent.estimated_time_sec}s
                  </div>
                )}
              </div>
            )}
      ...
      {/* ── Report Review Modal ── */}
      {showReviewModal && draftReport && (
        <div className="modal-overlay">
          <div className="modal-content">
            <div className="modal-header">
              <h2>Review Draft Report</h2>
              <button className="modal-close" onClick={() => setShowReviewModal(false)}>✕</button>
            </div>
            <div className="modal-body">
              <div className="report-field">
                <label>Study ID</label>
                <input type="text" value={draftReport.study_id} disabled />
              </div>
              <div className="report-field">
                <label>Findings</label>
                <textarea 
                  value={draftReport.findings} 
                  onChange={(e) => setDraftReport({...draftReport, findings: e.target.value})}
                  rows={8}
                />
              </div>
              <div className="report-field">
                ...
                <textarea 
                  value={draftReport.impression} 
                  onChange={(e) => setDraftReport({...draftReport, impression: e.target.value})}
                  rows={4}
            ...
            <div className="modal-footer">
              ...
              <button className="btn-primary" onClick={handleSignReport} disabled={reportLoading}>
                {reportLoading ? 'Signing...' : 'Sign & Finalize'}
              </button>
            </div>
          </div>
        </div>
